...
if '--h' in sys.argv or '--help' in sys.argv or len(sys.argv) != 4:
	print 'usage: Cart2Direct.py data_in lattice_in data_out'
	print 
	exit(1)
...
def get_rel_coord(abs_array, axes_array):
	"Use Ax = b to get relative coordinates array"
	abs_matrix = np.matrix(abs_array)
	axes_matrix = np.matrix(axes_array)

	return np.array(abs_matrix * axes_matrix.I)
...
rel_array = get_rel_coord(abs_array, axes_array)
